...
const PasswordChallenge: React.FC = () => {
  ...
  const [passwords, setPasswords] = useState(getRandomPasswords());
  ...
  const handleSubmitWeakPasswords = () => {
    // Get the indices of weak passwords in the current set
    const weakPasswordIndices = passwords
      .map((pwd, index) => pwd.weaknessReason !== null ? index : -1)
      .filter(index => index !== -1);
    
    // Check if all weak passwords are identified
    const allWeakIdentified = weakPasswordIndices.every(index => 
      identifiedWeakPasswords.includes(index)
    );
    
    if (allWeakIdentified) {
  ...
  const handleRetry = () => {
    setPasswords(getRandomPasswords());
    setIdentifiedWeakPasswords([]);
    setStage('identify');
  };
      ...
      {stage === 'create' && (
        ...
        >
          <div className="cyber-card mb-6">
            ...
            <div className="p-6">
              <div className="mb-6">
                <h3 className="text-sm text-cyber-light/80 mb-2 text-left">Password Requirements:</h3>
              ...
              {passwordFeedback && (
                <motion.div
                  ...
                  animate={{ opacity: 1, y: 0 }}
                  className={`p-4 rounded ${getStrengthColor(passwordFeedback.strength)}/10 border ${getStrengthColor(passwordFeedback.strength)}/20`}
                >
                  <div className="flex items-center mb-2">
                    <div className={`w-2 h-2 rounded-full ${getStrengthColor(passwordFeedback.strength)} mr-2`} />
                    <span className="font-medium capitalize">{passwordFeedback.strength} Password</span>
                  </div>
                  <ul className="text-left list-disc pl-3 space-y-0.5 text-sm">
          ...
          <div className="flex items-center">
            <button 
              ...
              disabled={!passwordFeedback?.isValid}
              className="cyber-button flex items-center"
            >
